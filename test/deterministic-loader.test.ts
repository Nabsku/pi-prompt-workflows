...
import { buildPromptCommandDescription, loadPromptsWithModel } from "../prompt-loader.ts";

function withTempHome(run: (root: string) => void) {
	const root = mkdtempSync(join(tmpdir(), "pi-prompt-workflows-deterministic-loader-"));
